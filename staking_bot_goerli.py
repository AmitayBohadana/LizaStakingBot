...
def handle_event(event, contract):
    try:
        ...
        total_staked_shares = total_staked_shares_raw / (10 ** 18)  # Convert from Wei to Ether
        total_staked_shares_formatted = f"{total_staked_shares:,.2f}"  # Format with comma separators and 2 decimal places

        # Check if price data is available
        if liza_usd_price is not None:
            usd_value = amount_staked * liza_usd_price
            usd_value_formatted = f"${usd_value:,.2f}"
            total_staked_usd_value = total_staked_shares * liza_usd_price
        ...
        etherscan_url = f"https://goerli.etherscan.io/tx/{transaction_hash_hex}"
